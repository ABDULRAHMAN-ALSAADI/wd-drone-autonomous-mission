...
This is the recommended operator flow for Mission 2.

## Important Principle

The Raspberry Pi runs only the Mission 2 target controller. Keep route
ownership simple:

1. Load the reviewed Mission 2 route in Mission Planner or QGroundControl.
2. Start the Mission 2 Pi profile.
3. Use the RC flight-mode switch to start AUTO.

The companion does not upload the route, arm, take off, or start AUTO.
...
That switch only allows target search after the Mission 2 profile is already
running.
...
back. It sends a zero-velocity command, clears the current target, and gives the
pilot control until the aircraft is disarmed.
